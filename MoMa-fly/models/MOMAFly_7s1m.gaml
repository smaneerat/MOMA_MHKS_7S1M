 ...
 *  =================================
 *  This version is based on  V4.5 optimized for OpenMole
 *  - R3-checkCurrentAct has been updated
 ...
 *  
 *  OPT 2 = modify save new aquatics stock! save to string in stead of writing directly to files
...
global //schedules: ([world] + SpatObj + shuffle(Aedes) + Stat)
{
	 ...
	 ***********************/
	 ...
	 bool bAutoSimStop <- true parameter: "auto stopping?" category: 'Sim Options';
	 ...
	 bool bDisprsShpNeed <- false parameter: "need dispersal shapefile/ show dispersal circle?"  category: 'Sim Options';
 	 bool bNeedNewZone <- false parameter: "need new study zone shapefile?" category: 'Sim Options';
	...
	/*******************************************************
	 * SPATOBJ Characteristics parameter to test in OPENMOLE
	 *******************************************************/
	float fgRecCapaRate  <- 1.6 parameter: "max human density? " category: 'Variable to test';
	float fgDenBsInNoWater <- 1.0 parameter: "density BS inside? " category: 'Variable to test';
	float fgDenBsOutNoWater <- 0.2 parameter: "density BS outside? " category: 'Variable to test';
	float fgPorosityAvg <- 0.5 parameter: "average porosity? " category: 'Variable to test';
	...
	string dispersShpName <- "dispersal.shp"; 
	string MaxDisPopShpName <- "Maxdispersal.shp";
	string newMapName <- "simHzrnsaket.shp";
	string deathFileName <- "deathOPT-fz0.csv";
	string fileDailyStock <- "OsDailyStcks-fz0.csv";
	string fileEggInWater <- "OsEinWater-fz0.csv";
	string fileEggLaid <- "OsEggLaid-fz0.csv";
	string fileNewLarvae <- "OsNewLar-fz0.csv";
	string fileNewPupae <- "OsNewPup-fz0.csv";
	string fileFuzzyProc <- "fuzzyTargetOPT-fz0.csv";
	string fileSelBestTarget <- "bestTargetOPT-fz0.csv";
	...
	int sim_days_duration <- 90; // How many days lates the simulation?
	 ...
	 *****************************************/
	reflex stopSimulation when: bAutoSimStop and nb_days = sim_days_duration or length(Aedes) = 0 //int(total_duration) > 72000000 //nb_days = sim_days_duration or length(Aedes) = 0
	{
...
//			}
		...
		action killMe(string cause)
		{
			...
			if(bCheckMaxDisp)
			{
				//save[bornObj, name, iAge, fMaxLifeDisps, fCoordX,fCoordY, cause] type: csv to: deathFileName;
		...
		action layEgg
		{
			...
			//------ crate new Aedes agent
			 if(bsToOvipo != nil)
			 {
				...
				//write  "to_ovipo: " + name + " eggsLaid : " + eggsLaid;
				...
				ask bsToOvipo
				{
					...
					if(iNbBsInWithWater + iNbBsOutWithWater > 0)
					{
				 	...
				 	//write "getCapaHaveEgg: " + getCapaHaveEgg();
					...
					if(eggsLaid < getCapaHaveEgg())//Aedes can lay all eggs
					{
					 	if(bLayInside)
					 	{
					 		...
					 		if(bCheckStck and eggsLaid != 0.0)
					 		{
					...
					else // lay only in the limit of the capacity having eggs
					{
						if(bLayInside)//rate to choose bs inside
						...
						else //lay egg to BS OUTSIDE
				 		{
					 		...
					 		if(bCheckStck and eggsLaid != 0.0)
					 		{
					 			save[nb_days, myself.name, iCapaHaveEgg, self.name, self.strClass, self.fSpatObjWaterTempCIn, "out"] type: csv to: fileEggLaid;
					 		}
				 		}
				...
				//write  name + " iWaitStckEggIn: " + iWaitStckEggIn + " iWaitStckEggOut: " + iWaitStckEggOut;
				}
			}			
		}
...
//			if(bCheckStck)//if save to files
//			{
//				save[nb_days, name, strClass, iNbNewFemaleA, sum(fListStkEtoLIn), sum(fListStkEtoLOut), sum(fListStkLtoPIn), sum(fListStkLtoPOut), sum(fListStkPtoAIn), sum(fListStkPtoAOut)] type: csv to: fileDailyStock;		 			
//			}	
...
//			}
		...
		action updateBsAttract //  = cal_iNbBsInWithWater
		{
				...
				int iInitBsIn <- iNbBsInNoWater;
				int iInitBsOut <- iNbBsOutNoWater;
...
//				else if (iNbBsOutWithWater < iNbBsOutNoWaterMin){iNbBsOutWithWater <- iNbBsOutNoWaterMin;}
				...
				{
					list_attrctTarget[mapTargetIndex["bs"]] <-iNbBsOutWithWater + iNbBsInWithWater;
				}
		}
		...
		float calRainEffRate
		{
			...
			else //no hatch if no rain or over flood in a BS a cause of heavy rainfall
			{
				fValue <- 0.0;
			}
			return fValue;
...
experiment 'Run this1 simulations' type: batch repeat: 1 keep_seed: true //until: total_duration = 10000 //nb_days = sim_days_duration or length(Aedes) = 0 
{
}
